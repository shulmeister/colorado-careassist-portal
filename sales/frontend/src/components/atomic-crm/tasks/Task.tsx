...
import { useDeleteWithUndoController, useGetIdentity, useNotify, useUpdate } from "ra-core";
...
import { getAssigneeLabel } from "./assignees";
...
export const Task = ({
  ...
  const meEmail = (identity as any)?.email ?? (identity?.id as string | undefined);
    ...
    <>
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-2 flex-1">
          ...
          />
          <div 
            className={`flex-grow cursor-pointer hover:text-primary transition-colors ${task.done_date ? "line-through" : ""}`}
            onClick={handleEdit}
          >
            ...
            <div className="text-sm text-muted-foreground">
              ...
              {task.assigned_to ? (
                <>
                  {" "}
                  • assigned to {getAssigneeLabel(task.assigned_to, meEmail)}
                </>
              ) : null}
              {showContact && (

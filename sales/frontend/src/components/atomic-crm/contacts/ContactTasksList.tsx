...
export const ContactTasksList = ({ contactId }: { contactId: Identifier }) => {
  ...
  const fetchTasks = async () => {
    try {
      const response = await fetch(
        `/sales/admin/contact-tasks?contact_id=${contactId}&sort=created_at&order=DESC`
      );
      if (response.ok) {
  ...
  // Set default assigned_to when identity loads
  useEffect(() => {
    if (identity && !newTask.assigned_to) {
      setNewTask(prev => ({ ...prev, assigned_to: (identity as any)?.email ?? "" }));
    }
  }, [identity]);

  const handleAddTask = async () => {
    try {
      const response = await fetch("/sales/admin/contact-tasks", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        notify(err.detail || "Failed to add task", { type: "error" });
      }
    } catch (error) {
  ...
  const handleToggleComplete = async (task: ContactTask) => {
    try {
      ...
      const response = await fetch(`/sales/admin/contact-tasks/${task.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...task,
          status: newStatus,
          done_date: newStatus === "done" ? new Date().toISOString() : null,
        }),
      });
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Add Task for Contact</DialogTitle>
          </DialogHeader>
          <div className="flex flex-col gap-4">
            <div>
...
const TaskItem = ({
  ...
  const taskText = task.text || task.title || task.description || "Untitled task";

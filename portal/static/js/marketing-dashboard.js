...
(function () {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    attachEventHandlers();
    const initialPreset =
      document.getElementById("datePreset")?.value || state.preset;
  ...
  function initTabNavigation() {
    ...
    let currentTabId =
      tabSections.find((section) => section.classList.contains("active"))?.id ||
      "tab-overview";
    ...
    sidebarLinks.forEach((link) => {
      link.addEventListener("click", (event) => {
        ...
        const rawTarget =
          link.getAttribute("data-tab-target") || link.getAttribute("data-tab");
  ...
  async function fetchDashboardData(preset) {
    ...
    try {
      const [
        ...
        fetchJson(`${API_BASE}/search-console?from=${start}&to=${end}`),
      ]);
      ...
      const engagement =
        engagementResult.status === "fulfilled" ? engagementResult.value : null;
      ...
      if (engagementResult.status === "rejected") {
        console.error(
          "Engagement metrics request failed",
          engagementResult.reason,
        );
      }
      if (gscResult.status === "rejected") {
        console.error(
          "Search Console metrics request failed",
          gscResult.reason,
        );
      }
  ...
  function renderDashboard({
    ...
    if (gsc?.success) {
      renderSearchConsoleSection(gsc, preset);
    }
  }
  ...
  function renderAdsSection(payload, preset) {
    ...
    const filteredFacebookCampaigns =
      filterCampaignsBySegment(facebookCampaigns);

    const googleSummary =
      state.segment === "all"
        ? google
        : buildGoogleMetricsFromCampaigns(
            filteredGoogleCampaigns,
            days,
            google,
          );
    ...
    updateCampaignEfficiencyChart(
      filteredGoogleCampaigns,
      filteredFacebookCampaigns,
    );
  ...
  function renderGoogleOverview(google) {
    ...
    setText(
      "googleAccountSpend",
      formatCurrency(google.spend?.total, 2, currency),
    );
    ...
    setText(
      "googleConvValue",
      formatCurrency(google.performance?.conversion_value, 2, currency),
    );

    const roasValue = safeNumber(google.efficiency?.roas);
    setText(
      "googleRoasMetric",
      roasValue !== null ? `${roasValue.toFixed(2)}x` : "—",
    );
    updatePill(
      "googleRoasSummary",
      roasValue !== null ? `ROAS ${roasValue.toFixed(2)}x` : "ROAS —",
    );
    ...
    setText(
      ...
      convRate !== null ? `${convRate.toFixed(2)}%` : "—",
    );

    const searchIS = safeNumber(google.efficiency?.search_impression_share);
    setText(
      "googleSearchIS",
      searchIS !== null ? `${searchIS.toFixed(1)}%` : "—",
    );

    const rankLostIS = safeNumber(
      google.efficiency?.search_rank_lost_impression_share,
    );
    setText(
      "googleRankLostIS",
      rankLostIS !== null ? `${rankLostIS.toFixed(1)}%` : "—",
    );

    const budgetLostIS = safeNumber(
      google.efficiency?.search_budget_lost_impression_share,
    );
    setText(
      "googleBudgetLostIS",
      budgetLostIS !== null ? `${budgetLostIS.toFixed(1)}%` : "—",
    );
    ...
    updatePill(
      ...
      costPerConv !== null
        ? `Cost/Conv ${formatCurrency(costPerConv, 2, currency)}`
        : "Cost/Conv —",
    );
  ...
  function renderGoogleSearchTerms(searchTerms) {
    ...
    container.innerHTML = searchTerms
      .slice(0, 10)
      .map((term) => {
  ...
  function renderGoogleDevicePerformance(devicePerf) {
    ...
    container.innerHTML = devices
      .map((device) => {
        ...
        const convRate = device.conversion_rate
          ? formatPercent(device.conversion_rate)
          : "—";
        return `
                <div class="stats-row">
  ...
  function renderFacebookOverview(account) {
    ...
    updatePill(
      "facebookCtrPill",
      account.ctr !== undefined ? `CTR ${formatPercent(account.ctr)}` : "CTR —",
    );
    updatePill(
      "facebookCpcPill",
      account.cpc !== undefined
        ? `CPC ${formatCurrency(account.cpc, 2)}`
        : "CPC —",
    );
    ...
    const change = calculateDailyTrend(
      account.daily || account.daily_breakdown,
    );
  ...
  function renderCampaignTables(googleCampaigns, facebookCampaigns) {
    ...
    if (googleBody) {
      if (!googleCampaigns.length) {
        ...
        googleBody.innerHTML = googleCampaigns
          .slice(0, 10)
          .map((campaign, index) => {
            const roasText = campaign.roas
    ...
    if (facebookBody) {
      if (!facebookCampaigns.length) {
        ...
        facebookBody.innerHTML = facebookCampaigns
          .slice(0, 6)
          .map(
            (campaign, index) => `
  ...
  function renderPaidSummary(google, facebookAccount, range, preset) {
    ...
    setTargetBadge(
      "paidTotalSpend",
      totalSpend,
      BENCHMARKS.targetDailySpend * ((range?.days || 30) / 30),
    );
    ...
    setText(
      "paidCPC",
      blendedCpc !== null ? formatCurrency(blendedCpc, 2) : "—",
    );
    setText("paidConversions", formatNumber(totalConversions));
    setTargetBadge(
      "paidConversions",
      totalConversions,
      BENCHMARKS.targetConversions,
    );
    ...
    const blendedCostPerConv = totalConversions
      ? totalSpend / totalConversions
      : null;
    setText(
      "paidCostPerConv",
      blendedCostPerConv !== null ? formatCurrency(blendedCostPerConv, 2) : "—",
    );
    ...
    setText(
      "paidCpm",
      blendedCpm !== null ? formatCurrency(blendedCpm, 2) : "—",
    );
  ...
  function evaluatePaidAlerts(metrics) {
    ...
    if (metrics.roasValue !== null && metrics.roasValue < 2) {
      alerts.push({
        level: "warning",
        message: `ROAS trending low at ${metrics.roasValue.toFixed(2)}x`,
      });
    }
    if (
      metrics.blendedCostPerConv !== null &&
      metrics.blendedCostPerConv > 300
    ) {
      alerts.push({
        level: "critical",
        message: `Cost per conversion spiked to ${formatCurrency(metrics.blendedCostPerConv, 2)}`,
      });
    }
    if (metrics.conversionRate !== null && metrics.conversionRate < 2) {
      alerts.push({
        level: "warning",
        message: `Conversion rate fell to ${metrics.conversionRate.toFixed(1)}%`,
      });
    }
    if (metrics.blendedCpm !== null && metrics.blendedCpm > 45) {
      alerts.push({
        level: "info",
        message: `CPM is elevated at ${formatCurrency(metrics.blendedCpm, 2)}`,
      });
    }
    if (metrics.facebookDelivery && metrics.facebookDelivery < 80) {
      alerts.push({
        level: "warning",
        message: `Facebook delivery rate down to ${metrics.facebookDelivery.toFixed(1)}%`,
      });
    }
    if (metrics.facebookCtr && metrics.facebookCtr < 0.5) {
      alerts.push({
        level: "info",
        message: `Facebook CTR is just ${metrics.facebookCtr.toFixed(2)}%`,
      });
    }
    return alerts;
  }
  ...
  function updatePaidCampaignStats(campaigns) {
    ...
    container.innerHTML = topCampaigns
      .map((campaign) => {
        ...
        const cpa = campaign.cost_per_conversion
          ? formatCurrency(campaign.cost_per_conversion, 2)
          : "—";
        return `
                    <div class="stats-row">
  ...
  function updateAdsCharts(google) {
    ...
    updateLineChart(
      charts.roas,
      labels,
      daily.map((entry) => roundNumber(entry.roas, 2)),
    );
    if (charts.roas?.data?.datasets?.[1]) {
      charts.roas.data.datasets[1].data = labels.map(
        () => BENCHMARKS.targetRoas,
      );
      charts.roas.update();
    }
    updateLineChart(
      charts.costPerf,
      labels,
      daily.map((entry) => roundNumber(entry.spend, 2)),
    );
    if (charts.costPerf?.data?.datasets?.[1]) {
      charts.costPerf.data.datasets[1].data = labels.map(
        () => BENCHMARKS.targetDailySpend,
      );
    ...
    updateLineChart(
      charts.costConv,
      labels,
      daily.map((entry) => roundNumber(entry.cost_per_conversion, 2)),
    );
    updateLineChart(
      charts.impressions,
      labels,
      daily.map((entry) => entry.impressions || 0),
    );
    updateLineChart(
      charts.interactionsClicks,
      labels,
      daily.map((entry) => entry.clicks || 0),
    );
    updateLineChart(
      charts.interactionsConv,
      labels,
      daily.map((entry) => entry.conversions || 0),
    );
  }
  ...
  function updatePaidDailyChart(google, facebook) {
    ...
    const googleDaily = Array.isArray(google?.spend?.daily)
      ? google.spend.daily
      : [];
    ...
    charts.paidDaily.data.datasets[0].data = dates.map(
      (date) => combined[date].googleSpend,
    );
    charts.paidDaily.data.datasets[1].data = dates.map(
      (date) => combined[date].facebookSpend,
    );
    charts.paidDaily.data.datasets[2].data = dates.map(
      (date) => combined[date].googleConversions,
    );
    charts.paidDaily.update();
  }
  ...
  function updateCampaignEfficiencyChart(googleCampaigns, facebookCampaigns) {
    ...
    charts.campaignEfficiency.data.datasets[0].data = (
      googleCampaigns || []
    ).map((campaign, index) => normalize(campaign, index, "google"));
    charts.campaignEfficiency.data.datasets[1].data = (
      facebookCampaigns || []
    ).map((campaign, index) => normalize(campaign, index, "facebook"));
  ...
  function renderOrganicSection(socialPayload, engagementPayload, preset) {
    ...
    if (socialPayload?.data) {
      ...
      renderFacebookDemographics(
        data.audience_demographics || summary.audience_demographics,
      );
  ...
  function renderPredisAIMetrics(predisData) {
    ...
    setText(
      "predisAIEngagementRate",
      formatPercent(predisData.avg_engagement_rate),
    );
    ...
    if (predisData.top_performing_post) {
      ...
      setText(
        "predisAITopPost",
        topPost.text ? topPost.text.substring(0, 50) + "..." : "N/A",
      );
    } else {
    ...
    if (platformsContainer && predisData.platform_breakdown) {
      let platformHtml = "";
      Object.entries(predisData.platform_breakdown).forEach(
        ([platform, count]) => {
          platformHtml += `
                    <div class="stats-row">
      ...
      platformsContainer.innerHTML =
        platformHtml ||
        '<div class="stats-row"><div class="stats-label">No data available</div></div>';
    }

    // Content type performance
    const contentTypesContainer = document.getElementById(
      "predisAIContentTypes",
    );
    if (contentTypesContainer && predisData.content_type_performance) {
      let contentHtml = "";
      Object.entries(predisData.content_type_performance).forEach(
        ([type, metrics]) => {
    ...
    if (recentContainer && predisData.recent_creations) {
      ...
      recentContainer.innerHTML =
        recentHtml ||
        '<div class="stats-row"><div class="stats-label">No recent creations</div></div>';
    }
  }
  ...
  function renderFacebookDemographics(demographics) {
    ...
    if (demographics.countries && demographics.countries.length) {
      html +=
        '<div class="stats-row" style="border-top: 1px solid #334155; margin-top: 6px; padding-top: 8px;"><div class="stats-label" style="font-weight: 600;">Top Countries</div></div>';
    ...
    if (demographics.cities && demographics.cities.length) {
      html +=
        '<div class="stats-row" style="border-top: 1px solid #334155; margin-top: 6px; padding-top: 8px;"><div class="stats-label" style="font-weight: 600;">Top Cities</div></div>';
    ...
    if (demographics.devices && Object.keys(demographics.devices).length) {
      html +=
        '<div class="stats-row" style="border-top: 1px solid #334155; margin-top: 6px; padding-top: 8px;"><div class="stats-label" style="font-weight: 600;">Devices</div></div>';
      Object.entries(demographics.devices)
        .slice(0, 3)
  ...
  function updateSocialCharts(dataPoints) {
    ...
    updateMultiDatasetChart(charts.socialReach, labels, [
      reachValues,
      engagementValues,
    ]);
  ...
  function renderWebsiteSection(payload, preset) {
    ...
    state.conversionPaths = Array.isArray(ga4.conversion_paths)
      ? ga4.conversion_paths
      : [];
  ...
  function renderGBPReviews(reviews) {
    ...
    if (
      ...
      Object.keys(reviews.rating_distribution).length
    ) {
      html +=
        '<div class="stats-row" style="border-top: 1px solid #334155; margin-top: 6px; padding-top: 8px;"><div class="stats-label" style="font-weight: 600;">Rating Distribution</div></div>';
    ...
    if (reviews.recent_reviews && reviews.recent_reviews.length) {
      html +=
        '<div class="stats-row" style="border-top: 1px solid #334155; margin-top: 6px; padding-top: 8px;"><div class="stats-label" style="font-weight: 600;">Recent Reviews</div></div>';
  ...
  function renderGBPPostEngagement(posts) {
    if (!posts || (posts.post_views === 0 && posts.post_engagement === 0))
      return;
  ...
  function renderGBPViewsBreakdown(gbp) {
    ...
    const searchPct =
      total > 0 ? (((gbp.views_search || 0) / total) * 100).toFixed(1) : 0;
    const mapsPct =
      total > 0 ? (((gbp.views_maps || 0) / total) * 100).toFixed(1) : 0;
  ...
  function renderEngagementSection(payload, preset) {
    ...
    setText(
      "engagementSaves",
      formatNumber(
        (sources.local?.directions || 0) + (sources.local?.website || 0),
      ),
    );
  ...
  function updateEngagementTypeChart(types) {
    ...
    charts.engagementType.data.datasets[0].backgroundColor = types.map(
      (t) => t.color,
    );
    charts.engagementType.update();
  }
    ...
    container.innerHTML = sources
      ...
      .map((source) => {
        ...
        const detailText = details.length
          ? `<span class="stats-change">${details.join(", ")}</span>`
          : "";
  ...
  function updateWebsiteCharts(ga4, gbp) {
    if (Array.isArray(ga4?.users_over_time) && ga4.users_over_time.length) {
      const labels = ga4.users_over_time.map((entry) =>
        formatChartLabel(entry.date),
      );
      updateLineChart(
        charts.sessions,
        labels,
        ga4.users_over_time.map((entry) => entry.users || 0),
      );
    }

    if (
      Array.isArray(ga4?.sessions_by_medium) &&
      ga4.sessions_by_medium.length
    ) {
      const labels = ga4.sessions_by_medium.map((entry) =>
        formatChartLabel(entry.date),
      );
      const datasetValues = ["none", "cpc", "paid", "referral", "organic"].map(
        (key) => ga4.sessions_by_medium.map((entry) => entry[key] || 0),
      );
    ...
    if (Array.isArray(gbp?.actions_over_time) && gbp.actions_over_time.length) {
      const labels = gbp.actions_over_time.map((entry) =>
        formatChartLabel(entry.date),
      );
      const datasetValues = [
  ...
  function updateAttributionChart() {
    ...
    const adConversions = google.spend.daily.map(
      (entry) => entry.conversions || 0,
    );
    const ga4Sessions = labels.map((date) => ga4Map[date] || 0);

    charts.attribution.data.labels = labels.map((date) =>
  ...
  function initCharts() {
    ...
    charts.impressions = createLineChart(
      "impressionsChart",
      "#38bdf8",
      "Impressions",
    );
    charts.sessions = createLineChart("sessionsChart", "#818cf8", "Sessions");
    charts.interactionsClicks = createLineChart(
      "interactionsChart1",
      "#3b82f6",
      "Clicks",
    );
    charts.interactionsConv = createLineChart(
      "interactionsChart2",
      "#f472b6",
      "Conversions",
    );
    ...
    charts.socialEngagement = createLineChart(
      "socialEngagementChart",
      "#fb923c",
      "Engagement",
    );
    charts.paidDaily = createPaidDailyChart("paidDailyChart");
    charts.campaignEfficiency = createCampaignEfficiencyChart(
    ...
    charts.emailGrowth = createLineChart(
      "emailGrowthChart",
      "#22c55e",
      "Subscribers",
    );
    ...
    charts.engagementTrend = createLineChart(
  ...
  function createMultiDatasetChart(canvasId, datasetConfigs, options = {}) {
    ...
    chart.__datasetKeys = datasetConfigs.map(
      (dataset) => dataset.key || dataset.label,
    );
    return chart;
  }
  ...
  function openCampaignDrilldown(sourceLabel, campaign) {
    ...
    const fields = [
      ...
      {
        label: "Cost / Conv.",
        value: campaign.cost_per_conversion
          ? formatCurrency(campaign.cost_per_conversion, 2)
          : "—",
      },
      {
      ...
      {
        label: "CTR",
        value: campaign.ctr ? `${campaign.ctr.toFixed(2)}%` : "—",
      },
    ];
  ...
  function recordRecentCampaign(sourceLabel, campaign) {
    ...
    state.recentCampaigns = [
      entry,
      ...state.recentCampaigns.filter((item) => item.name !== entry.name),
    ].slice(0, 5);
    renderRecentCampaigns();
  }
  ...
  function determineSegment(name = "") {
    ...
    if (
      SEGMENT_KEYWORDS.recruiting.some((keyword) => lower.includes(keyword))
    ) {
      return "recruiting";
    }
  ...
  function buildGoogleMetricsFromCampaigns(campaigns, days = 30, base) {
    ...
    return {
      ...
      efficiency: {
        ctr: roundNumber(
          totals.impressions ? (totals.clicks / totals.impressions) * 100 : 0,
          2,
        ),
        cpc: roundNumber(totals.clicks ? totals.spend / totals.clicks : 0, 2),
        cpm: roundNumber(
          totals.impressions ? (totals.spend / totals.impressions) * 1000 : 0,
          2,
        ),
        cost_per_conversion: roundNumber(
          totals.conversions ? totals.spend / totals.conversions : 0,
          2,
        ),
        roas: roundNumber(
          totals.spend ? totals.conversion_value / totals.spend : 0,
          2,
        ),
        conversion_rate: roundNumber(
          totals.clicks ? (totals.conversions / totals.clicks) * 100 : 0,
          2,
        ),
      },
      campaigns,
  ...
  function calculateDailyTrend(daily = []) {
    ...
    const firstAverage = average(
      firstSlice.map((entry) => Number(entry.spend) || 0),
    );
    const secondAverage = average(
      secondSlice.map((entry) => Number(entry.spend) || 0),
    );
  ...
  // ── Search Console (GSC) Section ─────────────────────────────

  function renderSearchConsoleSection(payload, preset) {
    if (!payload?.data) return;
    ...
    updateRangeLabel({
      ...
      preset,
      targetId: "gscDateRange",
    });

    // Show not-configured banner if applicable
    const banner = document.getElementById("gscNotConfigured");
    ...
    // KPI cards
    setText("gscTotalClicks", formatNumber(data.total_clicks));
    setText("gscTotalImpressions", formatNumber(data.total_impressions));
    setText("gscAvgCtr", data.avg_ctr !== undefined ? data.avg_ctr + "%" : "—");
    setText(
      "gscAvgPosition",
      data.avg_position !== undefined ? data.avg_position.toFixed(1) : "—",
    );

    // Trend chart
    renderGscTrendChart(data.daily_trend || []);

    // Top queries table
    renderGscTopQueries(data.top_queries || []);

    // Top pages table
    renderGscTopPages(data.top_pages || []);

    // Device breakdown chart
    renderGscDeviceChart(data.device_breakdown || []);

    // CTR trend chart
    renderGscCtrChart(data.daily_trend || []);
  }

  function renderGscTrendChart(daily) {
    ...
    if (charts.gscTrend) charts.gscTrend.destroy();

    charts.gscTrend = new Chart(canvas, {
      type: "line",
      data: {
        labels,
        datasets: [
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            position: "left",
            title: { display: true, text: "Clicks", color: "#60a5fa" },
            ticks: { color: "#60a5fa" },
            grid: { color: "rgba(51,65,85,0.3)" },
          },
          y1: {
            position: "right",
            title: { display: true, text: "Impressions", color: "#a78bfa" },
            ticks: { color: "#a78bfa" },
            grid: { drawOnChartArea: false },
          },
        },
  ...
  function renderGscCtrChart(daily) {
    const canvas = document.getElementById("gscCtrChart");
    if (!canvas) return;

    const labels = daily.map((d) => d.date);
    const ctr = daily.map((d) => d.ctr);

    if (charts.gscCtr) charts.gscCtr.destroy();

    charts.gscCtr = new Chart(canvas, {
      type: "line",
      data: {
        labels,
        datasets: [
  ...
  function renderGscDeviceChart(devices) {
    const canvas = document.getElementById("gscDeviceChart");
    if (!canvas) return;

    const labels = devices.map((d) => d.device);
    const clicks = devices.map((d) => d.clicks);
    const colors = ["#60a5fa", "#f59e0b", "#34d399", "#ef4444"];

    if (charts.gscDevice) charts.gscDevice.destroy();

    charts.gscDevice = new Chart(canvas, {
      type: "doughnut",
      data: {
        labels,
        datasets: [
  ...
  function renderGscTopQueries(queries) {
    const container = document.getElementById("gscTopQueries");
    if (!container) return;
    ...
    queries.slice(0, 15).forEach((q) => {
      ...
      label.style.flex = "2";
      label.textContent = q.query;

      const value = document.createElement("div");
      value.className = "stats-value";
      value.style.flex = "1";
      value.style.textAlign = "right";
      value.textContent = formatNumber(q.clicks);

      const change = document.createElement("span");
      change.className = "stats-change";
      change.style.fontSize = "11px";
      change.textContent = " " + q.ctr + "% CTR";
  ...
  function renderGscTopPages(pages) {
    const container = document.getElementById("gscTopPages");
    if (!container) return;
    ...
    pages.slice(0, 15).forEach((p) => {
      ...
      value.style.flex = "1";
      value.style.textAlign = "right";
      value.textContent = formatNumber(p.clicks);

      const change = document.createElement("span");
      change.className = "stats-change";
      change.style.fontSize = "11px";
      change.textContent = " pos " + p.position;
      value.appendChild(change);

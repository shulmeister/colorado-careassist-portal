...
class WellSkyService:
    ...
    def update_shift_status(
        ...
        shift_id: str,
        status: ShiftStatus,
        unassign_caregiver: bool = False,
        notes: str = None,
        call_out_reason: str = None,
        call_out_caregiver_id: str = None
    ) -> Tuple[bool, str]:
        """
        Update a shift's status in WellSky.

        This is the specific PUT request for WellSky ClearCare 'Update Shift' endpoint.
        Used by Gigi to mark shifts as 'Open' when a caregiver calls out.

        WellSky ClearCare API Endpoint: PUT /v1/shifts/{shift_id}

        Args:
            shift_id: The WellSky shift ID
            status: New status (e.g., ShiftStatus.OPEN for call-out)
            unassign_caregiver: If True, removes caregiver assignment
            notes: Optional notes to add to shift
            call_out_reason: Reason for call-out (sick, emergency, etc.)
            call_out_caregiver_id: ID of caregiver who called out
        ...
        if self.is_mock_mode:
            shift = self._mock_shifts.get(shift_id)
            if not shift:
                logger.warning(f"Mock: Shift {shift_id} not found")
                return False, f"Shift {shift_id} not found"

            # Store original caregiver for logging
            original_caregiver = f"{shift.caregiver_first_name} {shift.caregiver_last_name}"

            # Update shift status
            shift.status = status

            # Unassign caregiver if requested (for call-outs)
            if unassign_caregiver:
                shift.caregiver_id = None
                shift.caregiver_first_name = ""
                shift.caregiver_last_name = ""
            ...
            if notes:
                ...
                if call_out_reason:
                    new_note += f" | Call-out reason: {call_out_reason}"
                shift.notes = f"{shift.notes}\n{new_note}" if shift.notes else new_note

            logger.info(f"Mock: Updated shift {shift_id} status to {status.value}")
            return True, f"Shift updated to {status.value}. Previously assigned to {original_caregiver}"

        # Real WellSky API call
        # WellSky ClearCare API: PUT /v1/shifts/{shift_id}
        update_data = {
            "status": status.value,
        }

        if unassign_caregiver:
            update_data["caregiver_id"] = None

        if notes:
            update_data["notes"] = notes

        if call_out_reason:
            update_data["call_out_reason"] = call_out_reason
            update_data["call_out_time"] = datetime.now().isoformat()

        if call_out_caregiver_id:
            update_data["call_out_caregiver_id"] = call_out_caregiver_id
        ...
        if success:
            return True, f"Shift {shift_id} updated to {status.value}"
        else:
            return False, f"Failed to update shift: {response}"

    def mark_shift_open_for_callout(
        self,
        shift_id: str,
        call_out_caregiver_id: str,
        reason: str = "sick"
    ) -> Tuple[bool, str]:
        """
        Convenience method: Mark a shift as Open due to caregiver call-out.

        This is the primary method Gigi uses when processing call-outs.
        It unassigns the caregiver and sets status to Open.

        Args:
            shift_id: The shift ID
            call_out_caregiver_id: ID of the caregiver calling out
            reason: Call-out reason (sick, emergency, car_trouble, family, other)

        Returns:
            Tuple of (success: bool, message: str)
        """
        return self.update_shift_status(
            shift_id=shift_id,
            status=ShiftStatus.OPEN,
            unassign_caregiver=True,
            notes=f"Call-out reported via Gigi AI Agent",

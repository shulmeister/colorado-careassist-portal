...
class GoogleOAuthManager:
    ...
    async def handle_callback(self, code: str, state: str, request: Request = None) -> Dict[str, Any]:
        ...
        if request and hasattr(request, 'session'):
            ...
            if stored_state:
                if state != stored_state:
                    logger.warning(f"OAuth state mismatch - potential CSRF attack")
        ...
        try:
            ...
            # This is needed because Google may return additional scopes the user
            # has granted to other apps, causing scope mismatch errors with Flow.fetch_token()

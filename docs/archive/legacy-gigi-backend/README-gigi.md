...
```bash
# Run these commands on Mac Mini when going live:
# (obsolete) WELLSKY_CLIENT_ID=your_client_id
# (obsolete) WELLSKY_CLIENT_SECRET=your_client_secret
# (obsolete) WELLSKY_AGENCY_ID=your_agency_id
# (obsolete) WELLSKY_ENVIRONMENT=production

# ENABLE OPERATIONS (without this, notifications don't send!)
# (obsolete) GIGI_OPERATIONS_SMS_ENABLED=true

# After-hours only (SMS)
# (obsolete) GIGI_SMS_AUTOREPLY_ENABLED=true
# (obsolete) GIGI_SMS_AFTER_HOURS_ONLY=true
# (obsolete) GIGI_OFFICE_HOURS_START=08:00
# (obsolete) GIGI_OFFICE_HOURS_END=17:00
```
...
git add -A && git commit -m "Gigi changes" && git push origin main && git push origin main
```
...
curl -X POST "https://portal.coloradocareassist.com" \
...
```bash
# Create a memory
# (obsolete) "python gigi/memory_cli.py create 'Avoid scheduling on Sundays' --type explicit --category scheduling --impact high"

# List memories
# (obsolete) "python gigi/memory_cli.py list --category scheduling"

# Reinforce a memory
# (obsolete) "python gigi/memory_cli.py reinforce <memory_id>"

# Audit for conflicts
# (obsolete) "python gigi/memory_cli.py conflicts"
```

**Files:**
- `gigi/memory_system.py` - Core memory system
- `gigi/memory_cli.py` - CLI management tool
- `gigi/run_decay.py` - Daily decay script (Mac Mini Scheduler)
...
```bash
# Check current mode
# (obsolete) "python gigi/mode_cli.py current"

# Set mode manually
# (obsolete) "python gigi/mode_cli.py set focus --reason 'Important client call'"

# View mode history
# (obsolete) "python gigi/mode_cli.py history --hours 24"

# Get mode statistics
# (obsolete) "python gigi/mode_cli.py stats --days 7"
```
...
# (obsolete) "python gigi/failure_cli.py recent --hours 24"

# Filter by severity
# (obsolete) "python gigi/failure_cli.py recent --severity critical"

# Get failure statistics
# (obsolete) "python gigi/failure_cli.py stats --days 7"

# Resolve a failure
# (obsolete) "python gigi/failure_cli.py resolve <failure_id> 'Fixed by updating API key'"

# Check meltdown state
# (obsolete) "python gigi/failure_cli.py meltdown"

# Test the system
# (obsolete) "python gigi/failure_cli.py test"
```

...
5. Fill in app details:
   ...
   - **Redirect URI**: `https://portal.coloradocareassist.com`
...
1. Visit this URL (replace YOUR_CLIENT_ID):
   ...
   redirect_uri=https://portal.coloradocareassist.com&
   response_type=code
   ```
...
```bash
# (obsolete) QUICKBOOKS_CLIENT_ID=your_client_id -a careassist-tracker
# (obsolete) QUICKBOOKS_CLIENT_SECRET=your_client_secret -a careassist-tracker
# (obsolete) QUICKBOOKS_REALM_ID=your_realm_id -a careassist-tracker
# (obsolete) QUICKBOOKS_ACCESS_TOKEN=your_access_token -a careassist-tracker
# (obsolete) QUICKBOOKS_REFRESH_TOKEN=your_refresh_token -a careassist-tracker
```
...
```bash
# (obsolete) "python3 -c 'from quickbooks_service import QuickBooksService; qb = QuickBooksService(); result = qb.test_connection(); print(result)'" -a careassist-tracker
```

## Step 5: Run Sync

```bash
# (obsolete) "python3 sync_quickbooks_to_brevo.py" -a careassist-tracker
```
...
```bash
# (obsolete) scheduler:standard -a careassist-tracker
```

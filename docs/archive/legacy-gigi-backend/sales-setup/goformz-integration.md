...
**Manual Sync:**
```bash
# (obsolete) "python3 sync_goformz_to_brevo.py" -a careassist-tracker
```
...
**Or via API endpoint:**
```bash
curl -X POST https://portal.coloradocareassist.com?since_hours=24 \
...
```bash
# (obsolete) "python3 -c 'from goformz_service import GoFormzService; g = GoFormzService(); print(g.test_connection())'" -a careassist-tracker
```

### Test Sync (Dry Run)
```bash
# (obsolete) "python3 sync_goformz_to_brevo.py --since-hours 24" -a careassist-tracker
```
...
- Check that `GOFORMZ_CLIENT_ID` and `GOFORMZ_CLIENT_SECRET` are set on Mac Mini:
  ```bash
  # (obsolete) -a careassist-tracker | grep GOFORMZ
  ```
...
4. **Monitor logs** for any issues:
   ```bash
   # (obsolete) --tail -a careassist-tracker | grep goformz
   ```

...
| **SMS**             | RC message-store polling | `ringcentral_bot.py` | 43 tools | Working |
| **Direct Messages** | RC Glip API polling      | `ringcentral_bot.py` | 90 tools | Working |
| **Team Chat**       | RC Glip API polling      | `ringcentral_bot.py` | 90 tools | Working |
...
| **iMessage**               | BlueBubbles webhook      | `main.py` → `ask_gigi.py` | 90 tools | Code Done (needs BB GUI setup) |
...
- **All channels that use ask_gigi.py** get all 90 CANONICAL_TOOLS
- **Cross-channel context:** API messages visible from Telegram/SMS and vice versa

### Tool Architecture

All tool definitions live in `gigi/tool_registry.py` (90 CANONICAL_TOOLS). Channel filtering via `get_tools(channel)`:

- **Telegram / Ask-Gigi / DM / Team Chat**: All 90 tools
- **Voice**: 86 from registry (4 excluded) + 6 voice-only (`transfer_call`, `send_sms`, `send_team_message`, `send_email`, `lookup_caller`, `report_call_out`) = 92 total
- **SMS**: 43 tools (47 excluded — caregivers don't need browser, terminal, research tools)

Tool implementations in `gigi/tool_executor.py`. Voice-only tools handled locally in `voice_brain.py`. See `docs/GIGI_TOOL_ARCHITECTURE.md` for full details.
...
- **Learning Pipeline** (`gigi/learning_pipeline.py`): LLM-as-Judge scoring (accuracy, helpfulness, tone, tool_selection, safety). DB: `gigi_evaluations`. Cron every 6h. CLI: `python -m gigi.learning_pipeline [--hours N] [--limit N] [--channel CH]`. Portal → Gigi → Learning tab.
...
│   ├── tool_registry.py   # Single source of truth: 90 CANONICAL_TOOLS + channel filtering
│   ├── tool_executor.py   # Shared execute(name, input) for all tool implementations
│   ├── voice_brain.py     # Retell Custom LLM WebSocket handler (92 tools)
│   ├── telegram_bot.py    # Telegram interface (90 tools)
│   ├── ringcentral_bot.py # RC polling: SMS (43 tools), DM/Team (90 tools), scheduled messages
...
│   ├── learning_pipeline.py   # LLM-as-Judge evaluation (cron every 6h)
│   ├── travel_tools.py    # Sabre/Amadeus flights, hotel search, transfers
│   ├── finance_tools.py   # QuickBooks AR, P&L, balance sheet, invoices
│   ├── claude_code_tools.py   # Claude Code task creation/checking
...
│   ├── run_gigi_simulations.py  # 22 voice simulation scenarios + CLI runner

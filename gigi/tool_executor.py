...
async def execute(tool_name: str, tool_input: dict) -> str:
    ...
    try:
        ...
        # === WELLSKY / OPERATIONS ===
            ...
            def _clock_in():
                ...
                return {"success": success, "message": message,
                        "appointment_id": appointment_id, "caregiver_name": caregiver_name}
            ...
            def _find_replacement():
                try:
                    ...
                    campaign = shift_filling_engine.process_calloff(
                        ...
                        reason=reason, reported_by="gigi",
                    )
                    if not campaign:
        ...
        # === WEB / RESEARCH ===
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=150.0) as client:
                    resp = await client.post("http://localhost:3002/api/research/deep", json={"question": question})
                    data = resp.json()
                    ...
                    if isinstance(polybot_resp, Exception):
                        ...
                        if strat_perf:
                            ...
                            for name, sp in sorted(strat_perf.items(), key=lambda x: x[1].get("realized_pnl", 0), reverse=True):
                                report.append(f"  {name}: {sp.get('total_trades', 0)} trades, ${sp.get('realized_pnl', 0):+,.2f} P&L ({sp.get('win_rate', 0):.0f}% WR)")
                            report.append("")
                        report.append(f"Open Positions ({len(poly_positions)}):")
                        for p in poly_positions[:8]:
                            report.append(f"  {p.get('symbol', '?')[:50]}: {p.get('amount', 0):.1f} shares @ {p.get('entry_price', 0):.4f} (P&L: ${p.get('unrealized_pnl', 0):+.2f})")
                            ...
                            poly.update({
                                ...
                                "positions": [
                                    {"title": p.get("title", "?")[:60], "shares": p.get("shares", 0),
                                     ...
                                     "current_pct": round((p.get("current") or 0) * 100),
        ...
        # === FINANCE ===
            ...
            result = await asyncio.to_thread(get_subscription_audit, tool_input.get("months_back", 6))
            return json.dumps(result)
        ...
        # === FAX ===
            ...
            result = await _send_fax(to=tool_input.get("to", ""), media_url=tool_input.get("media_url", ""))
            return json.dumps(result)

        elif tool_name == "list_faxes":
            from services.fax_service import list_faxes as _list_faxes
            result = _list_faxes(direction=tool_input.get("direction"), limit=tool_input.get("limit", 10))
            return json.dumps(result)
        ...
        # === TICKET WATCHES ===
            ...
            if not cos_tools:
                return json.dumps({"error": "Chief of Staff tools not available."})
            result = await cos_tools.remove_ticket_watch(watch_id=tool_input.get("watch_id"))
            return json.dumps(result)

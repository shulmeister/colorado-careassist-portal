...
SHADOW_DM_CHAT_ID = os.getenv(
    "GIGI_SHADOW_DM_CHAT_ID", "1586118164482"
)  # Jason's DM chat

# LOOP PREVENTION - Critical safeguards
REPLY_COOLDOWN_MINUTES = (
    0.5  # Don't reply to same number within this window (30 seconds)
)
...
GIGI_SHIFT_MONITOR_ENABLED = (
    os.getenv("GIGI_SHIFT_MONITOR_ENABLED", "false").lower() == "true"
)
...
SMS_TOOLS = [
    ...
    {
        "name": "save_memory",
        "description": "Save an important operational fact, caregiver pattern, client preference, or instruction to long-term memory. Use when something notable emerges that should be remembered across conversations.",
        "input_schema": {
            "type": "object",
            "properties": {
                "content": {
                    "type": "string",
                    "description": "The fact, pattern, or preference to save.",
                },
                "category": {
                    "type": "string",
                    "description": "Category: scheduling, caregiver, client, operations, communication, general",
                },
    ...
    {
        "name": "clock_in_shift",
        "description": "Clock a caregiver into their shift in WellSky. Use when a caregiver texts that they forgot to clock in. Look up their shift first with get_wellsky_shifts.",
        "input_schema": {
            "type": "object",
            "properties": {
                "appointment_id": {
                    "type": "string",
                    "description": "Shift/appointment ID from WellSky",
                },
    ...
    },
    {
        ...
        "description": "Clock a caregiver out of their shift in WellSky. Use when a caregiver texts that they forgot to clock out. Look up their shift first with get_wellsky_shifts.",
        "input_schema": {
            "type": "object",
            "properties": {
                "appointment_id": {
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                "shift_id": {
                    "type": "string",
                    "description": "Shift/appointment ID needing coverage",
                },
    ...
    {
        "name": "get_task_board",
        "description": "Read Jason's task board. Shows tasks by section: Today, Soon, Later, Waiting, Agenda, Inbox, Done.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "add_task",
        "description": "Add a task to Jason's task board. Use when Jason texts 'I have a task', 'add to my list', 'remind me to'.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "section": {
                    "type": "string",
                    "description": "Board section: Today, Soon, Later, Waiting, Agenda, Inbox (default: Today)",
                },
            },
            "required": ["task"],
        },
    ...
    {
        "name": "complete_task",
        "description": "Mark a task done on Jason's task board.",
        "input_schema": {
            "type": "object",
            "properties": {
                "task_text": {
    ...
    {
        "name": "capture_note",
        "description": "Capture a quick note or idea to Jason's scratchpad. Use when Jason texts 'I have an idea', 'note this', 'jot this down'.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Read today's daily notes for context.",
        "input_schema": {
            "type": "object",
            "properties": {
                "date": {
...
DM_TOOLS = [
    ...
    {
        "name": "get_wellsky_clients",
        "description": "Search for clients in WellSky by name, or get all active clients.",
        "input_schema": {
            "type": "object",
            "properties": {
                "search_name": {
                    "type": "string",
                    "description": "Client name to search (leave empty for all)",
                },
                "active_only": {
                    "type": "boolean",
                    "description": "Only active clients (default true)",
                },
            },
            "required": [],
        },
    ...
    {
        "name": "get_wellsky_caregivers",
        "description": "Search for caregivers in WellSky by name, or get all active caregivers.",
        "input_schema": {
            "type": "object",
            "properties": {
                "search_name": {
                    "type": "string",
                    "description": "Caregiver name to search (leave empty for all)",
                },
                "active_only": {
                    "type": "boolean",
                    "description": "Only active caregivers (default true)",
                },
            },
            "required": [],
        },
    ...
    {
        "name": "get_wellsky_shifts",
        "description": "Get shifts from WellSky. Use get_wellsky_clients/caregivers first to find IDs.",
        "input_schema": {
            "type": "object",
            "properties": {
                ...
                "past_days": {
                    "type": "integer",
                    "description": "Days back for history/hours (default 0)",
                },
                "open_only": {
                    "type": "boolean",
                    "description": "Only open/unfilled shifts",
                },
            },
            "required": [],
        },
    ...
    {
        "name": "web_search",
        "description": "Search the internet for current information — news, sports, prices, general knowledge.",
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Get current stock price for a ticker symbol (AAPL, TSLA, etc.)",
        "input_schema": {
            "type": "object",
    ...
    },
    {
        ...
        "description": "Find upcoming concerts and live music events.",
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Artist, venue, or city to search",
                }
            },
            "required": ["query"],
        },
    },
    {
        ...
        "description": "Get upcoming events from Jason's Google Calendar.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "search_emails",
        "description": "Search Jason's Gmail.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "check_recent_sms",
        "description": "Check recent inbound SMS text messages across ALL company lines (307-459-8220, 719-428-3999). Shows texts from caregivers, clients, and anyone else who texted in.",
        "input_schema": {
            "type": "object",
            "properties": {
                "hours": {
                    "type": "integer",
                    "description": "How many hours back to look (default 12, max 48)",
                },
                "from_phone": {
                    "type": "string",
    ...
    {
        "name": "send_sms",
        "description": "Send an SMS text message from the company number (307-459-8220) to any phone number.",
        "input_schema": {
            "type": "object",
            "properties": {
                "to_phone": {
                    "type": "string",
                    "description": "Phone number to text (e.g., +13035551234)",
                },
                "message": {
                    "type": "string",
                    "description": "The SMS message to send (keep under 300 chars)",
                },
            },
    ...
    {
        "name": "log_call_out",
        "description": "Log a caregiver call-out as a Care Alert in WellSky and create a coverage task. Use when a caregiver reports they can't make their shift.",
        "input_schema": {
            "type": "object",
            "properties": {
                "caregiver_id": {
                ...
                "reason": {
                    "type": "string",
                    "description": "Reason for the call-out (e.g., 'sick', 'emergency', 'car trouble')",
                },
                "shift_date": {
                    "type": "string",
                    "description": "Date of the shift (YYYY-MM-DD, defaults to today)",
    ...
    {
        "name": "identify_caller",
        "description": "Look up who a phone number belongs to. Checks caregiver, client, staff, and family records in WellSky.",
        "input_schema": {
            "type": "object",
            "properties": {
                "phone_number": {
    ...
    },
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "extract_links": {
                    "type": "boolean",
                    "description": "Also extract links (default false)",
                },
            },
            "required": ["url"],
        },
    ...
    {
        "name": "deep_research",
        "description": "Run deep autonomous financial research using the Elite Trading platform's 40+ data tools and 9 AI agents. Use for ANY investment question: stock analysis, crypto, macro outlook, etc. Takes 30-120 seconds.",
        "input_schema": {
            "type": "object",
            "properties": {
                "question": {
    ...
    },
    {
        ...
        "description": "List all active ticket watches.",
        "input_schema": {"type": "object", "properties": {}, "required": []},
    },
    {
        "name": "remove_ticket_watch",
        "description": "Stop watching for tickets.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "clock_in_shift",
        "description": "Clock a caregiver into their shift in WellSky. Use when a caregiver needs help clocking in.",
        "input_schema": {
            "type": "object",
            "properties": {
                "appointment_id": {
    ...
    {
        "name": "clock_out_shift",
        "description": "Clock a caregiver out of their shift in WellSky. Use when a caregiver needs help clocking out.",
        "input_schema": {
            "type": "object",
            "properties": {
                "appointment_id": {
    ...
    {
        "name": "add_task",
        "description": "Add a task to Jason's task board. Use when Jason says 'I have a task', 'add to my list', 'remind me to'.",
        "input_schema": {
            "type": "object",
            "properties": {
                "task": {"type": "string", "description": "The task description"},
    ...
    {
        "name": "capture_note",
        "description": "Capture a quick note or idea to Jason's scratchpad.",
        "input_schema": {
            "type": "object",
            "properties": {
                "note": {"type": "string", "description": "The note or idea to capture"}
    ...
    # === GOOGLE WORKSPACE ADMIN (GAM) — READ ONLY ===
    {
        ...
        "description": "Query Google Workspace admin data using GAM. READ-ONLY — can look up users, groups, domains, devices, reports. Cannot create, update, delete, or modify anything. Examples: 'info user jacob@coloradocareassist.com', 'print users', 'print groups', 'info domain', 'report users'.",
        "input_schema": {
            "type": "object",
            "properties": {
                "command": {
                    "type": "string",
                    "description": "GAM command WITHOUT the leading 'gam'. Examples: 'info user jason@coloradocareassist.com', 'print users fields name,email,suspended', 'print groups', 'info domain', 'report users parameters accounts:last_login_time'. Only read commands (info, print, show, report, check) are allowed.",
                }
            },
            "required": ["command"],
        },
...
class GigiRingCentralBot:
    def __init__(self):
        ...
        self._reply_lock = (
            asyncio.Lock()
        )  # Prevent concurrent cooldown check + reply races
        ...
        if GIGI_SHIFT_MONITOR_ENABLED:
            ...
            logger.info(
                "Autonomous shift monitor disabled (set GIGI_SHIFT_MONITOR_ENABLED=true to enable)"
            )
        ...
        try:
            ...
            if CLOCK_REMINDER_ENABLED:
                self.clock_reminder = ClockReminderService(
                    wellsky_service=self.wellsky, sms_send_fn=self._send_sms_via_rc
                )
        ...
        if GIGI_SMS_SHADOW_MODE:
            logger.info(
                "📋 SMS SHADOW MODE: Draft replies will be reported to DM for review (not sent)"
            )
        else:
            logger.info("🟢 SMS LIVE MODE: Replies will be sent directly to callers")
        logger.info(
            f"Reply history loaded: {len(self.reply_history.get('replies', []))} recent replies tracked"
        )
        ...
        try:
            ...
            if not client_row:
                return json.dumps(
                    {
                        "status": "not_found",
                        "message": f"No active client matching '{client_name}'",
                    }
                )
            ...
            try:
                ...
                for s in live_shifts:
                    s_start = getattr(s, "start_datetime", None) or getattr(
                        s, "scheduled_start", None
                    )
                    s_end = getattr(s, "end_datetime", None) or getattr(
                        s, "scheduled_end", None
                    )
                    cg_name = getattr(s, "caregiver_name", None) or ""
                    cg_id = getattr(s, "caregiver_id", None) or getattr(
                        s, "practitioner_id", None
                    )
                    ...
                    if (
                        not s_start
                        and hasattr(s, "date")
                        and hasattr(s, "start_time")
                        and s.date
                        and s.start_time
                    ):
                        try:
                            s_start = datetime.combine(
                                s.date, datetime.strptime(s.start_time, "%H:%M").time()
                            )
                    ...
                    if (
                        not s_end
                        and hasattr(s, "date")
                        and hasattr(s, "end_time")
                        and s.date
                        and s.end_time
                    ):
                        try:
                            s_end = datetime.combine(
                                s.date, datetime.strptime(s.end_time, "%H:%M").time()
                            )
                    ...
                    if (
                        s_start
                        and s_end
                        and s_start <= now <= s_end
                        and cg_name
                        and cg_name != "Unassigned"
                    ):
                        result = {
                            "client": client_full_name,
                            "status": "active",
            ...
            if next_shifts:
                ns = next_shifts[0]
                next_info = {
                    "caregiver": ns[2] or "Unassigned",
                    "start": ns[0].strftime("%a %I:%M %p"),
                }
    ...
    def _can_reply_to_number(self, phone: str) -> tuple[bool, str]:
        ...
        hourly_reset = datetime.fromisoformat(
            self.reply_history.get("hourly_reset", now.isoformat())
        )
        ...
        if self.reply_history.get("hourly_count", 0) >= MAX_REPLIES_PER_HOUR_GLOBAL:
            return (
                False,
                f"Global hourly limit reached ({MAX_REPLIES_PER_HOUR_GLOBAL}/hr)",
            )
        ...
        for reply in self.reply_history.get("replies", []):
            ...
            if reply_phone == clean_phone:
                ...
                if minutes_since < REPLY_COOLDOWN_MINUTES:
                    return (
                        False,
                        f"Cooldown active ({int(REPLY_COOLDOWN_MINUTES - minutes_since)} min remaining)",
                    )
        ...
        if replies_to_number_today >= MAX_REPLIES_PER_DAY_PER_NUMBER:
            return (
                False,
                f"Daily limit reached for this number ({MAX_REPLIES_PER_DAY_PER_NUMBER}/day)",
            )
    ...
    def _record_reply(self, phone: str, reply_text: str = ""):
        ...
        self.reply_history["hourly_count"] = (
            self.reply_history.get("hourly_count", 0) + 1
        )
        ...
        while True:
            try:
                ...
                def on_sub_created(sub):
                    ...
                    logger.info(
                        f"📡 WebSocket subscription created: {info.get('id', 'unknown')}"
                    )
                        ...
                        if isinstance(parsed, list) and len(parsed) > 0:
                            ...
                            if msg_type == "ServerNotification":
                                ...
                                asyncio.ensure_future(
                                    self._handle_sms_notification(parsed)
                                )
                    except Exception as e:
                ...
                for change in changes:
                    ...
                    if change_type == "SMS" and new_ids:
                        ...
                        for new_msg_id in new_ids:
                            await self._fetch_and_process_sms(
                                str(new_msg_id), event_filter
                            )
                return
    ...
    async def _fetch_and_process_sms(self, msg_id: str, event_filter: str):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if (
                    sms_data.get("direction") == "Inbound"
                    and sms_data.get("type") == "SMS"
                ):
    ...
    async def _process_sms_record(self, body: dict, msg_id: str):
        ...
        to_phone = (
            body.get("to", [{}])[0].get("phoneNumber", "") if body.get("to") else ""
        )
        ...
        await self.process_documentation(
            sms_record, text, source_type="sms", phone=from_phone
        )
        ...
        if REPLIES_ENABLED:
            ...
            if from_phone not in own_numbers:
                await self.process_reply(
                    sms_record, text, reply_method="sms", phone=from_phone
                )
            else:
    ...
    def _get_admin_access_token(self):
        ...
        try:
            ...
            if not client_id or not client_secret:
                logger.error(
                    "RINGCENTRAL_CLIENT_ID or RINGCENTRAL_CLIENT_SECRET not set"
                )
                return None
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"JWT exchange failed: {response.status_code} - {response.text}"
                )
                return None
        except Exception as e:
    ...
    def _send_sms_via_rc(self, to_phone: str, message: str):
        ...
        if digits_only not in whitelist:
            logger.warning(
                f"SMS BLOCKED (not whitelisted): {to_phone} — message: {message[:80]}..."
            )
            return (
                False,
                f"Number {to_phone} not in SMS whitelist. Gigi outbound SMS is restricted.",
            )
    ...
    async def check_and_act(self):
        ...
        try:
            status = (
                "BUSINESS HOURS (Silent)"
                if self.is_business_hours()
                else "AFTER HOURS (Active)"
            )
            ...
            if GIGI_SHIFT_MONITOR_ENABLED and self._active_campaigns:
                now = datetime.utcnow()
                if (
                    now - self._last_campaign_check
                ).total_seconds() >= CAMPAIGN_CHECK_INTERVAL_SECONDS:
    ...
    def _load_last_notified_task_id(self) -> int:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT value FROM gigi_dedup_state WHERE key = 'last_notified_task_id'"
            )
            row = cur.fetchone()
            cur.close()
            conn.close()
    ...
    async def check_team_chats(self):
        ...
        messages = self.rc_service.get_chat_messages(
            chat["id"], since=datetime.utcnow() - timedelta(minutes=10), limit=20
        )
        ...
        for msg in messages:
            ...
            if creation_time_str:
                try:
                    creation_time = datetime.strptime(
                        creation_time_str, "%Y-%m-%dT%H:%M:%S.%fZ"
                    )
                except ValueError:
                    try:
                        creation_time = datetime.strptime(
            ...
            mentioned_names = (
                [m.get("name", "").lower() for m in mentions] if mentions else []
            )
            ...
            gigi_mentioned = "gigi" in text.lower() or any(
                "gigi" in n for n in mentioned_names
            )
            ...
            if gigi_mentioned or in_active_convo:
                sender_name = (
                    self._resolve_sender_name(creator_id)
                    if creator_id
                    else "Team member"
                )
                ...
                reply = await self._get_llm_dm_reply(
                    text, sender_name, f"team_{chat_id}"
                )
                if reply:
                    try:
                        ...
                        self._team_chat_active_conversations[creator_id] = (
                            datetime.utcnow()
                        )
                    except Exception as e:
        ...
        if new_msg_count == 0:
            logger.debug(
                f"Glip: All {len(messages)} messages in {TARGET_CHAT} were pre-startup or already processed"
            )
    ...
    async def check_direct_glip_messages(self):
        ...
        try:
            ...
            for chat in direct_chats:
                ...
                messages = self.rc_service.get_chat_messages(
                    chat_id, since=datetime.utcnow() - timedelta(minutes=10), limit=10
                )
                if not messages:
                    continue
                ...
                for msg in messages:
                    ...
                    logger.info(
                        f"Glip DM: New message from {sender_name} ({creator_id}) in chat {chat_id}: {text[:50]}..."
                    )
                    ...
                    if self.llm:
                        try:
                            reply = await self._get_llm_dm_reply(
                                text, sender_name, chat_id
                            )
                        except Exception as e:
                            logger.error(
                                f"LLM DM reply failed ({self.llm_provider}): {e}"
                            )
                    ...
                    try:
                        ...
                        if result:
                            logger.info(
                                f"Glip DM reply sent to chat {chat_id}: {reply[:50]}..."
                            )
                        else:
                            logger.error(
                                f"Failed to send Glip DM reply to chat {chat_id}"
                            )
                    except Exception as e:
    ...
    async def check_direct_sms(self):
        ...
        try:
            ...
            for ext_info in extensions_to_poll:
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"RC SMS Store Error ({label}): {response.status_code}"
                        )
                except Exception as e:
            ...
            for sms in all_records:
                ...
                # Role 1: Documenter
                await self.process_documentation(
                    sms, text, source_type="sms", phone=from_phone
                )
                ...
                if REPLIES_ENABLED:
                    ...
                    if from_phone not in own_numbers:
                        await self.process_reply(
                            sms, text, reply_method="sms", phone=from_phone
                        )
                    else:
                        logger.info(f"⏭️ Skipping reply to company number: {from_phone}")
    ...
    async def process_documentation(
        self, msg: dict, text: str, source_type: str = "chat", phone: str = None
    ):
            ...
            try:
                ...
                if len(clean_phone) == 10:
                    ...
                    if row:
                        ...
                        logger.info(
                            f"Doc: Identified SMS sender as caregiver: {caregiver_name}"
                        )
                conn.close()
            except Exception as e:
        ...
        if possible_names:
            try:
                ...
                for pname in possible_names:
                    ...
                    if name_parts and name_parts[0].lower() in (
                        "mrs",
                        "mr",
                        "ms",
                        "dr",
        ...
        try:
            ...
            if note_type == "callout":
                note_title = f"{sender} call-out" + (
                    f" — {client_name} shift" if client_name else ""
                )
            elif note_type == "late":
                note_title = f"{sender} running late" + (
                ...
                note_title = f"{sender} shift confirmed" + (
                    f" — {client_name}" if client_name else ""
                )
            elif note_type == "schedule_change":
                note_title = f"{sender} schedule change" + (
                    f" — {client_name}" if client_name else ""
                )
            else:
            ...
            note_body = (
                ...
                + (
                    ...
                    if caregiver_name and caregiver_name != sender
                    else ""
                )
                + f"\n{text}\n"
            )
            ...
            if client_id:
                try:
                    ...
                    if success and "WellSky" in str(result_msg):
                        ...
                        logger.info(
                            f"✅ Documented to WellSky for {client_name}: {result_msg}"
                        )
                    elif success:
                        logger.warning(
                            f"⚠️ Documented LOCALLY ONLY for {client_name}: {result_msg}"
                        )
                    else:
                        logger.error(
                            f"Documentation failed for {client_name}: {result_msg}"
                        )
                except Exception as e:
            ...
            # -----------------------------------------------------------------
                ...
                try:
                    ...
                    if shifts:
                        ...
                        for s in shifts:
                            sid = getattr(s, "client_id", None) or (
                                s.get("client_id") if isinstance(s, dict) else None
                            )
                            if sid:
                                shift_client_id = str(sid)
                                shift_client_name = getattr(s, "client_name", None) or (
                                    s.get("client") if isinstance(s, dict) else None
                                )
                                break
                        if shift_client_id:
                            ...
                            if success and "WellSky" in str(result_msg):
                                ...
                                logger.info(
                                    f"✅ Documented to WellSky via caregiver schedule → {client_name}: {result_msg}"
                                )
                            else:
                                logger.warning(
                                    f"⚠️ Caregiver doc for {caregiver_name} saved locally: {result_msg}"
                                )
                    else:
                        logger.info(
                            f"No shifts today for {caregiver_name} — documented in PostgreSQL only"
                        )
                except Exception as e:
                    logger.warning(
                        f"Caregiver→client schedule lookup for doc failed: {e}"
                    )
            ...
            try:
                ...
                person_type = (
                    "client"
                    if client_id
                    else ("caregiver" if caregiver_id else "unknown")
                )
                ...
                conn.close()
                logger.info(
                    f"Doc: PostgreSQL backup logged (synced={wellsky_synced}, type={note_type}, person={person_name_val})"
                )
            except Exception as e:
        ...
        if caregiver_id and self.preference_extractor:
            try:
                ...
                if memory_ids:
                    logger.info(
                        f"Extracted {len(memory_ids)} preferences for {caregiver_name}"
                    )
            except Exception as e:
        ...
        if note_type == "callout" and GIGI_SHIFT_MONITOR_ENABLED:
            await self._trigger_shift_filling(
                possible_names, caregiver_name if caregiver_id else None, text[:100]
            )
    ...
    async def _trigger_shift_filling(
        self, possible_names: list, caregiver_name: str, reason: str
    ):
        ...
        try:
            ...
            for name in possible_names:
                ...
                try:
                    ...
                    for p in practitioners:
                        if (
                            clean.lower() in p.full_name.lower()
                            or p.last_name.lower() == last_name.lower()
                        ):
                            caregiver_id = p.id
                            cg_name = p.full_name
                            break
            ...
            for shift in shifts:
                shift_status = (
                    shift.status.value
                    if hasattr(shift.status, "value")
                    else str(shift.status)
                )
                ...
                logger.info(
                    f"Shift monitor: Auto-triggering fill for shift {shift.id} ({cg_name} → {getattr(shift, 'client_name', 'unknown client')})"
                )
                ...
                try:
                    ...
                    if resp.status_code == 200:
                        ...
                        if campaign_id:
                            ...
                            logger.info(
                                f"Shift filling campaign started: {campaign_id}, contacted {data.get('candidates_contacted', 0)} caregivers"
                            )
                        ...
                        logger.error(
                            f"Shift filling API error: {resp.status_code} - {resp.text[:200]}"
                        )
                except Exception as e:
            ...
            try:
                ...
                if data.get("shift_filled"):
                    ...
                    if elapsed_min >= CAMPAIGN_ESCALATION_MINUTES:
                        logger.warning(
                            f"Campaign {campaign_id} unfilled after {elapsed_min:.0f} min — escalating"
                        )
                        try:
                            self.rc_service.send_message_to_chat(
                                TARGET_CHAT,
                        ...
                        completed.append(
                            campaign_id
                        )  # Stop tracking to avoid repeat notifications
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if data.get("calls_made", 0) > 0:
                        logger.info(
                            f"Voice follow-ups initiated: {data['calls_made']} calls"
                        )
            except Exception as e:
    ...
    async def _execute_sms_tool(
        self, tool_name: str, tool_input: dict, caller_phone: str = None
    ) -> str:
        ...
        try:
            if tool_name == "get_client_current_status":
                return self._get_client_current_status(
                ...
                try:
                    ...
                    for s in shifts or []:
                        ...
                        if sid:
                            shift_client_id = str(sid)
                            cname = getattr(s, "client_name", None) or (
                                s.get("client") if isinstance(s, dict) else None
                            )
                            if cname:
                    ...
                    if shift_client_id:
                        ...
                        logged_to = (
                            "WellSky Care Alert"
                            if success and "WellSky" in str(result_msg)
                            else "local (no encounter found)"
                        )
                    else:
                        logger.info(
                            f"No shifts today for {caregiver_name} — call-out logged locally"
                        )
                except Exception as e:
                ...
                return json.dumps(
                    {
                        "success": True,
                        "message": f"Call-out logged for {caregiver_name} ({logged_to}).",
                    }
                )
            ...
            if RC_FAILURE_HANDLER_AVAILABLE and _rc_failure_handler:
                try:
                    _rc_failure_handler.handle_tool_failure(
    ...
    async def _get_llm_sms_reply(self, text: str, phone: str) -> str:
        ...
        if RC_MEMORY_AVAILABLE and _rc_memory_system:
            try:
                memories = _rc_memory_system.query_memories(
                    min_confidence=0.5, limit=15
                )
                if memories:
        ...
        if clean_phone in ("3074598220",):
            xc = self.conversation_store.get_cross_channel_summary(
                "jason", "sms", limit=5, hours=24
            )
            if xc:
                system += xc
        ...
        try:
            ...
            logger.error(
                f"LLM SMS reply error ({self.llm_provider}): {e}", exc_info=True
            )
            return None
    ...
    async def _get_llm_dm_reply(self, text: str, sender_name: str, chat_id: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"LLM DM reply to {sender_name} ({self.llm_provider}, {len(final_text)} chars)"
            )
            return final_text

        except Exception as e:
            logger.error(
                f"LLM DM reply error ({self.llm_provider}): {e}", exc_info=True
            )
    ...
    async def _call_gemini(
        ...
        for m in conv_history:
            ...
            contents.append(
                genai_types.Content(
                    role=role, parts=[genai_types.Part(text=content_val)]
                )
            )
        ...
        if not final_text and tool_round >= LLM_MAX_TOOL_ROUNDS:
            logger.info(
                f"{channel} Gemini exhausted {tool_round} tool rounds with no text — forcing summary"
            )
            ...
            contents.append(
                genai_types.Content(
                    ...
                    parts=[
                        genai_types.Part(
                            text="Based on all the tool results above, please give a direct, complete answer to the original question. Be concise."
                        )
                    ],
                )
            )
            ...
            try:
                ...
                if (
                    summary_response.candidates
                    and summary_response.candidates[0].content
                    and summary_response.candidates[0].content.parts
                ):
                    ...
                    final_text = "".join(text_parts)
                    logger.info(
                        f"{channel} Gemini forced summary: {len(final_text)} chars"
                    )
            except Exception as e:
                logger.error(f"{channel} Gemini forced summary failed: {e}")

        logger.info(
            f"{channel} Gemini reply ({tool_round} tool rounds, {len(final_text)} chars)"
        )
        return final_text
    ...
    async def _call_anthropic(
        ...
        if not final_text and tool_round >= LLM_MAX_TOOL_ROUNDS:
            logger.info(
                f"{channel} Anthropic exhausted {tool_round} tool rounds — forcing text summary"
            )
            messages.append(
                {
                    "role": "assistant",
                    "content": [
                        {
                            "type": "text",
                            "text": "Based on the information I've gathered, let me summarize:",
                        }
                    ],
                }
            )
    ...
    async def _execute_dm_tool(self, tool_name: str, tool_input: dict) -> str:
        ...
        try:
            if tool_name == "get_client_current_status":
                ...
                if not token:
                    return json.dumps(
                        {"error": "Could not get RingCentral access token"}
                    )
                try:
                    params = {
                        "messageType": "SMS",
                    ...
                    for sms_item in sms_list:
                        try:
                            ...
                            if caller:
                                sms_item["sender_name"] = caller.get(
                                    "full_name", caller.get("name", "")
                                )
                    ...
                    return json.dumps(
                        {
                            "count": len(sms_list),
                            "hours_back": hours,
                            "lines_monitored": ["307-459-8220", "719-428-3999"],
                            "messages": sms_list,
                        }
                    )
                except Exception as e:
                ...
                if not to_phone or not message:
                    return json.dumps(
                        {"error": "Both to_phone and message are required"}
                    )
                success, result = self._send_sms_via_rc(to_phone, message)
                if success:
                    logger.info(
                        f"SMS sent from DM tool to {to_phone}: {message[:50]}..."
                    )
                    return json.dumps(
                        {"success": True, "message": f"SMS sent to {to_phone}"}
                    )
                else:
                ...
                try:
                    ...
                    if caller:
                        ...
                        result = {
                            "identified_as": type_map.get(
                                caller.get("type", ""), caller.get("type", "")
                            ),
                            "id": caller.get("id", ""),
                            "name": caller.get("full_name", caller.get("name", "")),
                            "phone": phone,
                ...
                return json.dumps(
                    {
                        "identified_as": "unknown",
                        "message": f"Phone number {phone} not found in records",
                    }
                )
    ...
    def _resolve_sender_name(self, creator_id: str) -> str:
        ...
        try:
            ext_info = self.rc_service._api_request(
                f"/account/~/extension/{creator_id}"
            )
            if ext_info:
    ...
    async def process_reply(
        self, msg: dict, text: str, reply_method: str = "chat", phone: str = None
    ):
    ...
    async def _process_reply_inner(
        self, msg: dict, text: str, reply_method: str = "chat", phone: str = None
    ):
        ...
        if reply_method == "sms" and phone:
            ...
            if not can_reply:
                logger.warning(
                    f"⛔ LOOP PREVENTION: Blocking reply to ...{phone[-4:]}. Reason: {reason}"
                )
                return
        ...
        if self.llm:
            try:
                ...
                logger.error(
                    f"LLM reply failed ({self.llm_provider}), falling back to static: {e}"
                )
                reply = None
        ...
        if not reply:
            ...
            if "call out" in lower_text or "sick" in lower_text:
                ...
                "help" in lower_text
                or "shift" in lower_text
                or "question" in lower_text
            ):
        ...
        if reply and reply_method == "sms" and phone:
            if self._detect_semantic_loop(phone, reply):
                logger.warning(
                    f"🔁 SEMANTIC LOOP detected for ...{phone[-4:]} — forcing escalation reply"
                )
                ...
                if reply_method == "chat":
                    ...
                    if GIGI_SMS_SHADOW_MODE:
                        ...
                        try:
                            ...
                            logger.info(
                                f"👻 Shadow Mode: Draft SMS to {clean_phone} reported to DM (not sent)"
                            )
                        except Exception as e:
                    ...
                    logger.info(
                        f"Sending SMS reply to {clean_phone} via {RINGCENTRAL_FROM_NUMBER} (using Admin Token)"
                    )
                    ...
                    response = requests.post(
                        url, headers=headers, json=data, timeout=20
                    )
                    if response.status_code == 200:
                        ...
                        logger.error(
                            f"Failed to send SMS reply: {response.status_code} - {response.text}"
                        )
